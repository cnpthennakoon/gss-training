@extends('layouts.master-data')

@section('content')
    <div class="flex-container">

        <nav class="breadcrumb has-arrow-separator" aria-label="breadcrumbs">
            <ul>
                <li><a href="{{ route('home') }}">Home</a></li>
                <li><a href="{{ route('master-data') }}">Master Data</a></li>
                <li class="is-active"><a href="#" aria-current="page">Projects</a></li>
            </ul>
        </nav>

        <div class="columns">
            <div class="column">

                <h1 class="title">
                    Projects
                </h1>
            </div>
            <div class="column">
                <a href="{{ route('project.create') }}" class="button is-primary is-outlined is-pulled-right m-r-10">Add New Project</a>
            </div>
        </div>

        <div class="card">
            <div class="card-content">


                <nav class="level">
                    <!-- Left side -->
                    <div class="level-left">
                        <div class="level-item">
                            {!! Form::open(['method' => 'GET', 'url' => '/master-data/project', 'role'=>'search']) !!}
                            <div class="field has-addons">
                                <p class="control">
                                    <input class="input" name="project" type="text" placeholder="Find by project (slug)">
                                </p>
                                <p class="control">
                                    <input class="input" name="region" type="text" placeholder="Find by region">
                                </p>
                                <p class="control">
                                    <button class="button">
                                        Search
                                    </button>
                                </p>
                            </div>
                            {!! Form::close() !!}
                        </div>
                    </div>

                </nav>


                <table class="table is-bordered is-narrow is-fullwidth is-hoverable">
                    <thead>
                    <tr>
                        <th>Name</th>
                        <th>Slug</th>
                        <th>Global Project Name</th>
                        <th>Country</th>
                        <th>Region</th>
                        <th>Manufacturer</th>
                        <th>Created By</th>
                        <th>Created at</th>
                        <th>Last Updated at</th>
                        <th>Action</th>
                    </tr>
                    </thead>

                    <tbody>
                    @foreach($projects as $project)
                        <tr>
                            <td>{{ $project->name }}</td>
                            <td><span class="tag is-info">{{ $project->slug }}</span></td>
                            <td>{{ $project->globalProject->name }}</td>
                            <td>{{ $project->country->name }}</td>
                            <td>{{ $project->region->name }}</td>
                            <td>{{ $project->manufacturer->name }}</td>
                            <td>{{ $project->user->name }} <small>( {{ $project->user->email }} )</small></td>
                            <td>
                                @if($project->created_at)
                                    {{ $project->created_at->toDateString() }}
                                @endif
                            </td>
                            <td>
                                @if($project->updated_at)
                                    {{ $project->updated_at->toDateString() }}
                                @endif
                            </td>

                            <td class="has-text-centered">
                                {{--                                <a class="button is-outlined is-small is-half" href="{{ route('region.show', $country->id) }}">View</a>--}}
                                <a class="button is-outlined is-small is-half" href="{{ route('project.edit', $project->id) }}">Edit</a>
                            </td>
                        </tr>
                    @endforeach
                    </tbody>

                </table>

                <div class="text-center">
                    {!! $projects->links('vendor.pagination.bulma') !!}
                </div>

            </div>
        </div>



    </div>
@endsection

@section('scripts')

    <script>
        var app = new Vue({
            el: '#app',
            data: {
            }
        })
    </script>

@endsection